    ...
    private void setBtnDatHang(){
        btnDatHang.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                List<GioHang> gioHangs = new ArrayList<GioHang>();
                gioHangs = gioHangDAO.layGioHangTheoEmailRCVGioHanggh(email);
                SanPhamDAO sanPhamDAO = new SanPhamDAO(DatHangActivity.this);
                ...
                for (int i = 0; i < gioHangs.size(); i++){
                    ...
                    sanPhams = sanPhamDAO.layHetSanPhamTheoMSP(gioHang.getMaSp());
                    SanPham sanPham = sanPhams.get(0);
                    sanPham.setMaSanPham(sanPham.getMaSanPham());
                    sanPham.setMaLoai(sanPham.getMaLoai());
                    sanPham.setGiaThanh(sanPham.getGiaThanh());
                    sanPham.setHuongDan(sanPham.getHuongDan());
                    sanPham.setLinkUrl(sanPham.getLinkUrl());
                    sanPham.setMoTa(sanPham.getMoTa());
                    sanPham.setNhaCungCap(sanPham.getNhaCungCap());
                    sanPham.setTenSanPham(sanPham.getTenSanPham());
                    sanPham.setThuongHieu(sanPham.getThuongHieu());
                    sanPham.setTrongLuong(sanPham.getTrongLuong());
                    soLuongGioHang = Integer.parseInt(gioHang.getSoLuong());
                    soLuongFinal = sanPham.getSoLuong() - soLuongGioHang;
                    sanPham.setSoLuong(soLuongFinal);
